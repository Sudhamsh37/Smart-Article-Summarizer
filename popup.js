  ...
  resultDiv.innerHTML = '<div class="loading"><div class="loader"></div></div>';
    ...
    chrome.storage.sync.get(["geminiApiKey"], async (result) => {
        ...
        if (!result.geminiApiKey) {
            resultDiv.textContent = "NO API key found..Click the gear icon to add one.."
            return;
        }

        // 2.Ask content.js for the page text..

        // Uses Chrome's API to get the currently active tab in the current window.
        // {active: true, currentWindow: true} ensures it only gets the tab the user is looking at.
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            ...
            chrome.tabs.sendMessage(
                ...
                async (res) => {
                    ...
                    try {
                        const summary = await getGeminiSummary(res.text, summaryType, geminiApiKey);
